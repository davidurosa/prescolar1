@extends('layouts.app', ['class' => 'off-canvas-sidebar', 'activePage' => 'login', 'title' => __('Prescolar')])

@section('content')
<div class="container" style="height: auto;">
  <div class="row align-items-center">
    <div class="col-md-9 ml-auto mr-auto mb-3 text-center">
      <h3>{{ __('Inicie sesión para ver los estudiantes del prescolar') }} </h3>
    </div>
    <div class="col-lg-4 col-md-6 col-sm-8 ml-auto mr-auto">
      <form class="form" method="POST" action="{{ route('login') }}">
        ...
        <div class="card card-login card-hidden mb-3">
          ...
          <div class="card-body">
            <p class="card-description text-center">{{ __('El usuario  Administrador es: ') }} <strong>admin@material.com</strong> {{ __(' Y la contraseña  ') }}<strong>secret</strong> </p>
        ...
        <div class="col-6">
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="text-light">
                    <small>{{ __('Se te olvido tu contraseña?') }}</small>
                </a>
            @endif
        </div>
